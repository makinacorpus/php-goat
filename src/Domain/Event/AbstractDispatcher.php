...
abstract class AbstractDispatcher implements Dispatcher, LoggerAwareInterface
{
     ...
     */
    private function requeue(MessageEnvelope $envelope, RetryStrategyResponse $response): MessageEnvelope
    {
        ...
        try {
            ...
            try {
                if ($e instanceof DispatcherRetryableError) {
                    $response = $e->getRetryStrategyResponse();

                    $envelope = $this->requeue($envelope, $response);
